...
cache_get_file_path() {
  local cache_temp_dir="${1:?cache_temp_dir parameter required}"
  local job_id="${2:?job_id parameter required}"
  echo "${cache_temp_dir}/chinmina-oidc-${job_id}.cache"
}
...
cache_read() {
  ...
  cache_file="$(cache_get_file_path "${cache_temp_dir}" "${job_id}")"
